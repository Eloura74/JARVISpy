...
class MoonrakerClient:
    ...
    def get(self, path: str) -> dict:
        """Effectue un GET sur l'API Moonraker. Retourne le dict JSON."""
        if not self.is_configured:
            return {"error": "MOONRAKER_URL non configuré dans les paramètres."}
        try:
            ...
            return {"error": f"Impossible de joindre Moonraker à {self.base_url}. Vérifiez l'URL et le réseau."}
        except Exception as e:
            logger.error(f"Moonraker GET {path}: {e}")
            return {"error": str(e)}

    def post(self, path: str, body: dict | None = None) -> dict:
        """Effectue un POST sur l'API Moonraker."""
        if not self.is_configured:
            return {"error": "MOONRAKER_URL non configuré dans les paramètres."}
        try:
            url = f"{self.base_url}{path}"
            ...
            return {"error": f"Impossible de joindre Moonraker à {self.base_url}."}
        except Exception as e:
