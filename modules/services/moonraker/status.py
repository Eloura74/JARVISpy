...
async def get_printer_status() -> str:
    ...
    """
    data = await moonraker_client.get(
    ...
    summary = {
        ...
        "fichier": stats.get("filename", "aucun"),
        "durée_impression": f"{int(stats.get('print_duration', 0) // 60)} min",
        "extrudeur": {
            "actuel": round(extruder.get("temperature", 0), 1),
            "cible": round(extruder.get("target", 0), 1),
        },
        "plateau": {
            "actuel": round(bed.get("temperature", 0), 1),
            "cible": round(bed.get("target", 0), 1),
        },
        "avancement_%": round(display.get("progress", 0) * 100, 1),
    }

    # Signal pour le HUD (Désormais awaité car la fonction est async)
    await bus.emit("printer.status", summary)

    return json.dumps(summary, ensure_ascii=False)


async def get_print_progress() -> str:
    """
